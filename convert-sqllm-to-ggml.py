...
DT_Q4_SQ = SQDataType('SQ4')
DT_Q3_SQ = SQDataType('SQ3')
DT_Q4_8_SQ = SQDataType('SQ4_8')

DataType = Union[UnquantizedDataType, QuantizedDataType, SQDataType]
...
class GGMLFileType(enum.Enum):
    ...
    MostlyQ4_SQ = 16  # except 1d tensors
    MostlyQ3_SQ = 17  # except 1d tensors
    MostlyQ4_8_SQ = 18  # except 1d tensors
    ...
    def type_for_tensor(self, name: str, tensor: 'LazyTensor') -> DataType:
        if len(tensor.shape) == 1:
            ...
            if name in ('output.weight', 'tok_embeddings.weight'):
                ...
                return DT_Q4_1
        elif self == GGMLFileType.MostlyQ4_SQ:
            if name in ('output.weight', 'tok_embeddings.weight'):
                # return DT_F32
                return DT_F16
            else:
                return DT_Q4_SQ
        elif self == GGMLFileType.MostlyQ3_SQ:
            if name in ('output.weight', 'tok_embeddings.weight'):
                # return DT_F32
                return DT_F16
            else:
                return DT_Q3_SQ
        elif self == GGMLFileType.MostlyQ4_8_SQ:
            if name in ('output.weight', 'tok_embeddings.weight'):
                return DT_F16
            else:
                return DT_Q4_8_SQ
...
class GGMLSQTensor(Tensor):
    data_type: SQDataType
    ...
    def permute(self, n_head: int) -> 'GGMLSQTensor':
        return GGMLSQTensor(permute(self.ndarray, n_head), self.shape, self.data_type)

GGMLCompatibleTensor = Union[UnquantizedTensor, GGMLQuantizedTensor, GGMLSQTensor]
...
class GPTQForLLaMaQuantizedTensorSQ(Tensor):
    def __init__(self, model: 'LazyModel', namebase: str) -> None:
        qweight = load_unquantized(model[f"{namebase}.qweight"], np.int32)
        lut = load_unquantized(model[f"{namebase}.lookup_table"], np.float16, convert=True)
        ...
        self.data_type = SQDataType('SQ4') #TODO
        self.shape = [self.qweight.shape[0], self.qweight.shape[1] * 8]
    ...
    def to_ggml(self) -> GGMLSQTensor:
        # The output format looks like this:
        # For each row:
        #     - lut (fp16 * 16, 2 * 16 bytes)
        #     - weights (int4 * 8 * rowlen, 4 * rowlen bytes)

        lut_view = self.lut.view(dtype=np.int32)
        qweight_view = self.qweight.view(dtype=np.int32)
        dt = 'SQ4'
        grouped = np.concatenate([lut_view, qweight_view], axis=-1, casting='no')
        return GGMLSQTensor(grouped, self.shape, dt)
...
def handle_quantization(model: LazyModel) -> LazyModel:
    ...
    def convert(name: str) -> Tuple[str, LazyTensor]:
        if name.endswith(".qweight"):
            ...
            # actually loaded).
            lazy_lut = model[f"{namebase}.lookup_table"]

            real_shape = [lazy_tensor.shape[1], lazy_tensor.shape[0] * 8]
            data_type = SQDataType('SQ4')

            def load() -> Tensor:
                return GPTQForLLaMaQuantizedTensorSQ(model, namebase)

            out = (orig_name, LazyTensor(load, real_shape, data_type, '[quantized]'))
...
def lazy_load_ggml_file(fp: io.BufferedReader, path: Path) -> ModelPlus:
    ...
    def read_tensor() -> None:  # this is a function so that variables captured in `load` don't change
        ...
        def load() -> Tensor:
            ...
            elif isinstance(data_type, SQDataType):
                ndarray = np.frombuffer(buf, dtype=np.uint32)
                return GGMLSQTensor(ndarray, shape, data_type)
            else:
                return UnquantizedTensor(np.frombuffer(buf, dtype=numpy_dtype).reshape(shape))
...
def pick_output_type(model: LazyModel, output_type_str: Optional[str]) -> GGMLFileType:
    ...
    if output_type_str == "q4_sq":
        return GGMLFileType.MostlyQ4_SQ
...
def filter_and_sort_tensors(model: LazyModel) -> LazyModel:
    filtered = {name: model[name] for name in TENSORS_LIST if name in model}
    return filtered
...
def default_outfile(model_paths: List[Path], file_type: GGMLFileType) -> Path:
    namestr = {
        GGMLFileType.MostlyQ4_SQ: "q4_sq"
    }[file_type]
...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument("--outtype", choices=["q4_sq"], help="output format (default: based on input)")

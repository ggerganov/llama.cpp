...
from typing import IO, TYPE_CHECKING, Any, Callable, Generator, Iterable, Literal, Sequence, TypeVar, Union
...
@dataclass(frozen=True)
class SQDataType(DataType):
    ggml_type: gguf.GGMLQuantizationType
    def elements_to_bytes(self, tensor_shape) -> int:
        n_elems = np.prod(tensor_shape)/8
        n_elems += tensor_shape[0] * 8 # add 16 fp16 elements per row
        # print(tensor_shape)
        # print(n_elems)
        return n_elems.astype(np.int32) * 4
    pass

DT_Q4_SQ = SQDataType('SQ4',
    ggml_type = gguf.GGMLQuantizationType.Q4_SQ,
...
class GGMLFileType(enum.IntEnum):
    ...
    MostlyQ4_SQ = 19  # except 1d tensors

    def type_for_tensor(self, name: str, tensor: LazyTensor, equant) -> DataType:
        dt = GGML_FILE_TYPE_TO_DATA_TYPE.get(self)
        if dt is None:
            raise ValueError(self)
        if dt == DT_Q4_SQ:
            if name in ('output.weight', 'token_embd.weight') and equant:
                return DT_Q8_0
            elif name in ('output.weight', 'token_embd.weight'):
                return DT_F16
...
GGML_FILE_TYPE_TO_DATA_TYPE: dict[GGMLFileType, DataType] = {
    ...
    GGMLFileType.MostlyQ4_SQ: DT_Q4_SQ,
}
...
class SQTensor(Tensor):
    ...
    def astype(self, data_type: DataType) -> Tensor:
        ...
        qweights = self.ndarray[:, :-8]
        lut = self.ndarray[:, -8:].astype(np.float16)
        dq = dequantize_lut(qweights, lut) #TODO: not implemented for 3-bit

        return UnquantizedTensor(dq).astype(data_type)

    def permute(self, n_head: int, n_head_kv) -> 'SQTensor':
        return SQTensor(permute(self.ndarray, n_head, n_head_kv), self.shape, self.data_type)
...
GGMLCompatibleTensor = Union[UnquantizedTensor, SQTensor]
...
class OutputFile:
    ...
    def add_tensor_info(self, name: str, tensor: LazyTensor) -> None:
        ...
        if raw_dtype == gguf.GGMLQuantizationType.Q4_SQ:
            data_nbytes = tensor.data_type.elements_to_bytes(tensor.shape)
        else:
...
def convert_to_output_type(model: LazyModel, output_type: GGMLFileType, equant) -> LazyModel:
    return {name: tensor.astype(output_type.type_for_tensor(name, tensor, equant))
            for (name, tensor) in model.items()}

# extra GPTQForLLaMa methods for file conversion
...
class GPTQForLLaMaQuantizedTensorSQ(Tensor):
    def __init__(self, model: 'LazyModel', namebase: str) -> None:
        ...
        qweight = load_unquantized(model[f"{namebase}.qweight"], expected_dtype=np.int32)
        lut = load_unquantized(model[f"{namebase}.lookup_table"], expected_dtype=np.float16, convert=True)
    ...
    def permute(self, n_head: int, n_head_kv: int) -> Tensor:
        return DeferredPermutedTensor(self, n_head, n_head_kv)
    ...
    def to_ggml(self) -> SQTensor:
        ...
        return SQTensor(grouped, self.shape, dt)
...
def convert_model_names(model: LazyModel, params: Params, squeezellm = False) -> LazyModel:
...
def main(args_in: list[str] | None = None) -> None:
    ...
    parser.add_argument("--outtype",     choices=["f32", "f16", "q8_0", "q4_sq"], help="output format - note: q8_0 may be very slow (default: f16 or f32 based on input)")
    ...
    parser.add_argument("--equant",  action="store_true",    help="Convert to SQLLM")
    ...
    if args.outtype:
        params.ftype = {
            ...
            "q4_sq": GGMLFileType.MostlyQ4_SQ,
        }[args.outtype]
    ...
    model   = convert_model_names(model, params, args.squeezellm)
    ftype   = pick_output_type(model, args.outtype)
    model   = convert_to_output_type(model, ftype, args.equant)

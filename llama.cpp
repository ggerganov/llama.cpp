...
struct llm_build_llama_ctx : llm_build_ctx {
    ...
    void build_pre_repeating_kq_pos() {
...
struct llm_build_refact_ctx : llm_build_llama_ctx {
    llm_build_refact_ctx(llama_context & lctx, const llama_batch & batch)
            : llm_build_llama_ctx(lctx, batch)
    {}

    void build_pre_repeating_kq_pos() {
        // Pass, doesn't use RoPE.
    }
...
static struct ggml_cgraph * llm_build_refact(
    ...
    llm_build_refact_ctx bctx(lctx, batch);
    return bctx.build();
}

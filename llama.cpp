...
struct llm_tokenizer_bpe {
    ...
    void tokenize(const std::string & text, std::vector<llama_vocab::id> & output) {
        ...
        switch (vocab.type) {
            case LLAMA_VOCAB_TYPE_BPE:
                word_collection = unicode_regex_split(text, gpt2_regex);
                break;
            case LLAMA_VOCAB_TYPE_DEEPSEEKCODER:
                word_collection = unicode_regex_split(text, deepseek_coder_regex);
                break;
            case LLAMA_VOCAB_TYPE_DEEPSEEKLLM:
                word_collection = unicode_regex_split(text, deepseek_llm_regex);
                break;
            default:
                break;
        }
...
private:
    ...
    const std::vector<std::wstring> deepseek_coder_regex = {
    L"[\r\n]",
    //\s?\p{L}+
    ...
    const std::vector<std::wstring> deepseek_llm_regex = {
    L"[\r\n]",

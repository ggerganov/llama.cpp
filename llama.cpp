    ...
    struct ggml_cgraph * build_chatglm() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                cb(cur, "bqkv", il);
/*
                struct ggml_tensor * Qcur = ggml_cont(ctx0, ggml_view_3d(ctx0, cur,
                    n_embd_head, n_head, ne1,
                    n_embd_head * ggml_element_size(cur), cur->nb[1],
                    0));
                struct ggml_tensor * Kcur = ggml_cont(ctx0, ggml_view_3d(ctx0, cur,
                    n_embd_head, n_head_kv, ne1,
                    n_embd_head * ggml_element_size(cur), cur->nb[1],
                    ne0 * ggml_element_size(cur)));
                struct ggml_tensor * Vcur = ggml_cont(ctx0, ggml_view_3d(ctx0, cur,
                    n_embd_head, n_head_kv, ne1,
                    n_embd_head * ggml_element_size(cur), cur->nb[1],
                    (ne0 + n_embd_head * n_head_kv) * ggml_element_size(cur)));
*/
                Qcur = ggml_cont(ctx0, ggml_view_2d(ctx0, cur, n_embd,     n_tokens, cur->nb[1], 0*sizeof(float)*(n_embd)));
                Kcur = ggml_cont(ctx0, ggml_view_2d(ctx0, cur, n_embd_gqa, n_tokens, cur->nb[1], 1*sizeof(float)*(n_embd)));
                Vcur = ggml_cont(ctx0, ggml_view_2d(ctx0, cur, n_embd_gqa, n_tokens, cur->nb[1], 1*sizeof(float)*(n_embd + n_embd_gqa)));

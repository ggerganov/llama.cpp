...
struct llama_model_loader {
    ...
    struct ggml_tensor * get_tensor(int layer_num, const std::string & name, const std::vector<uint32_t> & ne, ggml_backend backend) {
        ...
        if (it == tensors_map.name_to_idx.end()) {
            throw std::runtime_error(std::runtime_error(format("llama.cpp: tensor '%s' is missing from model", layer_name_wnum.c_str())));
        }
...
#define LLAMA_BACKEND_OFFLOAD_SPLIT GGML_BACKEND_CPU
    ...
    size_t vram_scratch = 0;
    {
        ...
        for (uint32_t i = 0; i < n_layer; ++i) {
            ...
            layer.wq = ml->get_tensor(i, ".attention.wq.weight", {n_embd, n_embd}, backend_split);
            layer.wk = ml->get_tensor(i, ".attention.wk.weight", {n_embd, n_embd}, backend_split);
            layer.wv = ml->get_tensor(i, ".attention.wv.weight", {n_embd, n_embd}, backend_split);
            layer.wo = ml->get_tensor(i, ".attention.wo.weight", {n_embd, n_embd}, backend_split);

            layer.ffn_norm = ml->get_tensor(i, ".ffn_norm.weight", {n_embd}, backend);

            layer.w1 = ml->get_tensor(i, ".feed_forward.w1.weight", {n_embd,   n_ff},   backend_split);
            layer.w2 = ml->get_tensor(i, ".feed_forward.w2.weight", {  n_ff,   n_embd}, backend_split);
            layer.w3 = ml->get_tensor(i, ".feed_forward.w3.weight", {n_embd,   n_ff},   backend_split);

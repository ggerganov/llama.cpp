...
static bool llama_kv_cache_init(const struct llama_hparams &hparams,
                                struct llama_kv_cache &cache, ggml_type ktype,
                                ggml_type vtype, uint32_t n_ctx,
                                int n_gpu_layers, bool offload) {
  ...
  cache.buf.resize(n_elements *
                       (ggml_type_sizef(ktype) + ggml_type_sizef(vtype)) +
...
template <>
struct GKV_Base<uint8_t>
    : GKV_Base_Type<uint8_t, GGUF_TYPE_UINT8, gguf_get_val_u8> {};
template <>
struct GKV_Base<uint16_t>
    : GKV_Base_Type<uint16_t, GGUF_TYPE_UINT16, gguf_get_val_u16> {};
template <>
struct GKV_Base<uint32_t>
    : GKV_Base_Type<uint32_t, GGUF_TYPE_UINT32, gguf_get_val_u32> {};
template <>
struct GKV_Base<uint64_t>
    : GKV_Base_Type<uint64_t, GGUF_TYPE_UINT64, gguf_get_val_u64> {};
template <>
struct GKV_Base<int8_t>
    : GKV_Base_Type<int8_t, GGUF_TYPE_INT8, gguf_get_val_i8> {};
template <>
struct GKV_Base<int16_t>
    : GKV_Base_Type<int16_t, GGUF_TYPE_INT16, gguf_get_val_i16> {};
template <>
struct GKV_Base<int32_t>
    : GKV_Base_Type<int32_t, GGUF_TYPE_INT32, gguf_get_val_i32> {};
template <>
struct GKV_Base<int64_t>
    : GKV_Base_Type<int64_t, GGUF_TYPE_INT64, gguf_get_val_i64> {};
template <>
struct GKV_Base<float>
    : GKV_Base_Type<float, GGUF_TYPE_FLOAT32, gguf_get_val_f32> {};
template <>
struct GKV_Base<double>
    : GKV_Base_Type<double, GGUF_TYPE_FLOAT64, gguf_get_val_f64> {};
template <>
...
public:
  static T get_kv(const gguf_context *ctx, const int k) {
    ...
    if (kt != GKV::gt) {
      throw std::runtime_error(format(
          "key %s has wrong type %s but expected type %s", gguf_get_key(ctx, k),
  ...
  static const char *
  override_type_to_str(const llama_model_kv_override_type ty) {
    switch (ty) {
  ...
  static bool
  validate_override(const llama_model_kv_override_type expected_type,
                    const struct llama_model_kv_override *override) {
    if (!override) {
      return false;
    }
    if (override->tag == expected_type) {
      LLAMA_LOG_INFO("%s: Using metadata override (%5s) '%s' = ", __func__,
    ...
    LLAMA_LOG_WARN("%s: Warning: Bad metadata override type for key '%s', "
                   "expected %s but got %s\n",
                   __func__, override->key, override_type_to_str(expected_type),
                   override_type_to_str(override->tag));
    return false;
  }
  ...
  template <typename OT>
  static
      typename std::enable_if<std::is_same<OT, std::string>::value, bool>::type
      try_override(T &target, const struct llama_model_kv_override *override) {
    (void)target;
    (void) override;
    if (!override) {
    ...
    throw std::runtime_error(format(
        "Unsupported attempt to override string type for metadata key %s\n",
        override ? override->key : "NULL"));
  }

  static bool set(const gguf_context *ctx, const int k, T &target,
                  const struct llama_model_kv_override *override = nullptr) {
...
struct llama_model_loader {
  ...
  llama_model_loader(const std::string &fname, bool use_mmap,
    ...
    // TODO: make optional
    {
      ...
      LLAMA_LOG_INFO("%s: Dumping metadata keys/values. Note: KV overrides do "
      ...
      for (int i = 0; i < n_kv; i++) {
        ...
        const std::string type_name =
            type == GGUF_TYPE_ARRAY
                ? format("%s[%s,%d]", gguf_type_name(type),
                         gguf_type_name(gguf_get_arr_type(ctx_gguf, i)),
                         gguf_get_arr_n(ctx_gguf, i))
                : gguf_type_name(type);
  ...
  get_arr_n(const std::string &key, T &result, const bool required = true) {
    ...
    if (kid < 0) {
      if (required) {
        throw std::runtime_error(
            format("key not found in model: %s", key.c_str()));
      }
      return false;
    }
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        if (backend == GGML_BACKEND_GPU) {
          vram_weights +=
              ...
              ggml_nbytes(layer.wo) + (layer.bq ? ggml_nbytes(layer.bq) : 0) +
    ...
    case LLM_ARCH_QWEN: {
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        if (backend == GGML_BACKEND_GPU) {
          vram_weights +=
              ggml_nbytes(layer.attn_norm) + ggml_nbytes(layer.wqkv) +
              ggml_nbytes(layer.bqkv) + ggml_nbytes(layer.wo) +
              ggml_nbytes(layer.ffn_norm) + ggml_nbytes(layer.ffn_gate) +
              ggml_nbytes(layer.ffn_down) + ggml_nbytes(layer.ffn_up);
        }
      }
...
static void llm_build_k_shift(struct ggml_context *ctx,
                              ...
                              int64_t n_ctx, int n_rot, float freq_base,
  ...
  for (int il = 0; il < n_layer; ++il) {
    struct ggml_tensor *tmp =
        ...
        ggml_rope_custom_inplace(
            ctx,
            ggml_view_3d(ctx, kv.k_l[il], n_embd_head, n_head_kv, n_ctx,
                         ggml_type_sizef(kv.k_l[il]->type) * n_embd_head,
                         ggml_type_sizef(kv.k_l[il]->type) * n_embd_gqa, 0),
...
llm_build_kv_store(struct ggml_context *ctx, const llama_hparams &hparams,
  ...
  struct ggml_tensor *k_cache_view =
      ggml_view_1d(ctx, kv.k_l[il], n_tokens * n_embd_gqa,
                   (ggml_type_sizef(kv.k_l[il]->type) * n_embd_gqa) * kv_head);
  cb(k_cache_view, "k_cache_view", il);

  struct ggml_tensor *v_cache_view =
      ggml_view_2d(ctx, kv.v_l[il], n_tokens, n_embd_gqa,
...
llm_build_kqv(struct ggml_context *ctx, const llama_hparams &hparams,
  ...
  struct ggml_tensor *k =
      ggml_view_3d(ctx, kv.k_l[il], n_embd_head, n_kv, n_head_kv,
                   ggml_type_sizef(kv.k_l[il]->type) * n_embd_gqa,
                   ggml_type_sizef(kv.k_l[il]->type) * n_embd_head, 0);
  cb(k, "k", il);
  ...
  if (max_alibi_bias > 0.0f) {
    // temporary branch until we figure out how to handle ggml_alibi through
    // ggml_add
  ...
  struct ggml_tensor *v =
      ggml_view_3d(ctx, kv.v_l[il], n_kv, n_embd_head, n_head_kv,
                   ggml_element_size(kv.v_l[il]) * n_ctx,
                   ggml_element_size(kv.v_l[il]) * n_ctx * n_embd_head, 0);
  cb(v, "v", il);
  ...
  struct ggml_cgraph *build_stablelm() {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self-attention
      {
        ...
        Qcur = ggml_rope_custom(
            ctx0, ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head, n_tokens),
            inp_pos, hparams.n_rot, 2, 0, n_orig_ctx, freq_base, freq_scale,
            ext_factor, attn_factor, beta_fast, beta_slow);
        cb(Qcur, "Qcur", il);
...
static const std::unordered_map<const char *, llm_offload_func_e>
    k_offload_map = {
        ...
        {"inp_pos",
         OFFLOAD_FUNC_FRC}, // this is often used for KQ ops (e.g. rope)
...
#else
            {OFFLOAD_FUNC, "CPU"},       {OFFLOAD_FUNC_FRC, "CPU"},
            {OFFLOAD_FUNC_KQV, "CPU"},   {OFFLOAD_FUNC_NR, "CPU"},
    ...
    for (const auto &fragment : fragment_buffer) {
      if (fragment.type == FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT) {
        ...
        if (&fragment == &fragment_buffer.front()) {
          raw_text =
              " " +
              raw_text; // prefix with space if the first token is not special
        }
...
llama_model_quantize_internal(const std::string &fname_inp,
  ...
  for (int i = 0; i < ml.n_tensors; ++i) {
    ...
    if (!quantize) {
      ...
      LLAMA_LOG_INFO("size = %8.2f MiB -> %8.2f MiB | hist: ",
                     ggml_nbytes(tensor) / 1024.0 / 1024.0,
...
llama_new_context_with_model(struct llama_model *model,
  ...
  if (!hparams.vocab_only) {
...

    {
      ...
      LLAMA_LOG_INFO(
          "%s: KV self size  = %7.2f MiB, K (%s): %7.2f MiB, V (%s): %7.2f "
          "MiB\n",
          __func__,
...

    {
      ...
      LLAMA_LOG_INFO("%s: total VRAM used: %.2f MiB (model: %.2f MiB, context: "
                     "%.2f MiB)\n",
...
int llama_model_meta_val_str(const struct llama_model *model, const char *key,
...
int llama_model_meta_key_by_index(const struct llama_model *model, int i,
                                  char *buf, size_t buf_size) {
...
int llama_model_meta_val_str_by_index(const struct llama_model *model, int i,
                                      char *buf, size_t buf_size) {
  if (i < 0 || i >= (int)model->gguf_kv.size()) {
    if (buf_size > 0) {
      buf[0] = '\0';
...
void llama_kv_cache_view_update(const struct llama_context *ctx,
  ...
  if (uint32_t(view->n_cells) < ctx->kv_self.size || view->cells == nullptr) {
    ...
    p = realloc(view->cells_sequences,
                sizeof(llama_seq_id) * view->n_max_seq * view->n_cells);
  ...
  // copy kv cache
  {
    ...
    if (kv_buf_size) {
      ...
      ggml_context *cpy_ctx = ggml_init(
          {6 * n_layer * ggml_tensor_overhead() + ggml_graph_overhead(), NULL,
           /* no_alloc */ true});
      ...
      for (int il = 0; il < (int)n_layer; ++il) {
        ggml_tensor *kout2d =
            ggml_new_tensor_2d(cpy_ctx, kv_self.k_l[il]->type, n_embd, kv_head);
        kout2d_data[il].resize(ggml_nbytes(kout2d));
        kout2d->data = kout2d_data[il].data();

        ggml_tensor *vout2d =
            ggml_new_tensor_2d(cpy_ctx, kv_self.v_l[il]->type, kv_head, n_embd);
        vout2d_data[il].resize(ggml_nbytes(vout2d));
        vout2d->data = vout2d_data[il].data();

        ggml_tensor *k2d = ggml_view_2d(cpy_ctx, kv_self.k_l[il], n_embd,
                                        kv_head, elt_size * n_embd, 0);

        ggml_tensor *v2d = ggml_view_2d(cpy_ctx, kv_self.v_l[il], kv_head,
                                        n_embd, elt_size * n_ctx, 0);
...
#ifdef GGML_USE_METAL
  ggml_metal_log_set_callback(g_state.log_callback,

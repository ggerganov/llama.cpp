...
size_t ggml_v1_nbytes(const struct ggml_v1_tensor * tensor) {
    ...
    return ggml_v1_nelements(tensor)*GGML_V1_TYPE_SIZE[tensor->type];
}
...
struct ggml_v1_tensor * ggml_v1_new_tensor_impl(
    ...
    if (data == NULL) {
        size_needed += GGML_V1_TYPE_SIZE[type];
    ...
    *result = (struct ggml_v1_tensor) {
    ...
    for (int i = 1; i < GGML_V1_MAX_DIMS; i++) {
...
struct ggml_v1_tensor * ggml_v1_add_impl(
        ...
        bool inplace) {
    assert(ggml_v1_are_same_shape(a, b));
...
struct ggml_v1_tensor * ggml_v1_mul_mat(
        ...
        struct ggml_v1_tensor  * a,
        struct ggml_v1_tensor  * b) {
    assert(ggml_v1_can_mul_mat(a, b));
...
struct ggml_v1_tensor * ggml_v1_scale_impl(
        ...
        struct ggml_v1_tensor  * b,
        bool inplace) {
    assert(ggml_v1_is_scalar(b));
    assert(ggml_v1_is_padded_1d(a));
...
struct ggml_v1_tensor * ggml_v1_cpy_impl(
        ...
        struct ggml_v1_tensor  * b,
        bool inplace) {
    assert(ggml_v1_nelements(a) == ggml_v1_nelements(b));
...
struct ggml_v1_tensor * ggml_v1_reshape(
        ...
        struct ggml_v1_tensor * a,
        struct ggml_v1_tensor * b) {
    assert(ggml_v1_is_contiguous(a));
    assert(ggml_v1_is_contiguous(b));
    assert(ggml_v1_nelements(a) == ggml_v1_nelements(b));
...
struct ggml_v1_tensor * ggml_v1_reshape_2d(
        ...
        int                   ne1) {
    assert(ggml_v1_is_contiguous(a));
    assert(ggml_v1_nelements(a) == ne0*ne1);
...
struct ggml_v1_tensor * ggml_v1_reshape_3d(
        ...
        int                   ne2) {
    assert(ggml_v1_is_contiguous(a));
    assert(ggml_v1_nelements(a) == ne0*ne1*ne2);
...
struct ggml_v1_tensor * ggml_v1_permute(
        ...
        int                   axis2,
        int                   axis3) {
    assert(axis0 >= 0 && axis0 < GGML_V1_MAX_DIMS);
    assert(axis1 >= 0 && axis1 < GGML_V1_MAX_DIMS);
    assert(axis2 >= 0 && axis2 < GGML_V1_MAX_DIMS);
    assert(axis3 >= 0 && axis3 < GGML_V1_MAX_DIMS);
...
struct ggml_v1_tensor * ggml_v1_get_rows(
        ...
        struct ggml_v1_tensor  * a,
        struct ggml_v1_tensor  * b) {
    assert(ggml_v1_is_matrix(a) && ggml_v1_is_vector(b) && b->type == GGML_V1_TYPE_I32);
...
struct ggml_v1_tensor * ggml_v1_flash_attn(
        ...
        bool                  masked) {
    assert(ggml_v1_can_mul_mat(k, q));
...
struct ggml_v1_tensor * ggml_v1_flash_ff(
    ...
    assert(ggml_v1_can_mul_mat(b0, a));
    // TODO: more checks
...
static void ggml_v1_compute_forward_dup_f16(
    ...
    assert(params->ith == 0);
    assert(ggml_v1_is_contiguous(dst));
    assert(ggml_v1_nelements(dst) == ggml_v1_nelements(src0));
...
static bool ggml_v1_compute_forward_mul_mat_use_blas(
    ...
    if (ggml_v1_is_contiguous(src0) && ggml_v1_is_contiguous(src1) && (
...
static void ggml_v1_visit_parents(struct ggml_v1_cgraph * cgraph, struct ggml_v1_tensor * node) {
    ...
    if (node->op == GGML_V1_OP_NONE && node->grad == NULL) {
        ...
        assert(cgraph->n_leafs < GGML_V1_MAX_NODES);

        cgraph->leafs[cgraph->n_leafs] = node;
        cgraph->n_leafs++;
    } else {
        assert(cgraph->n_nodes < GGML_V1_MAX_NODES);
                        ...
                        if (node->src0->nb[1] < node->src0->nb[0]) {
                            ...
                            if (node->src0->type == GGML_V1_TYPE_F16 &&
                                ...
                                if (ggml_v1_compute_forward_mul_mat_use_blas(node->src0, node->src1, node)) {
                                    ...
                                    cur = sizeof(ggml_v1_fp16_t)*ggml_v1_nelements(node->src1);
                                }
#else
                                cur = sizeof(ggml_v1_fp16_t)*ggml_v1_nelements(node->src1);
#endif

...
{
    return gpt_params_parse_with_extra_check(argc, argv, params, nullptr);
}

bool check_is_extra_args(std::string arg, std::vector<std::string> *extra_args)
{
...
bool gpt_params_parse_with_extra_check(int argc, char **argv, gpt_params &params, std::vector<std::string> *extra_args)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            if (check_is_extra_args(argv[i], extra_args) == true)
            {
                if (i+1 < argc)
                {
                    std::string content = argv[i+1];
                    if (content.compare(0, arg_prefix.size(), arg_prefix) != 0)
                    {
                        // is content for extra_arg
                        i++;
                    }
                }
            }
        ...
        else if (extra_args == nullptr || check_is_extra_args(argv[i], extra_args) == false)
        {
            fprintf(stderr, "error: unknown argument: %s\n", arg.c_str());
            gpt_print_usage(argc, argv, default_params);
            exit(1);
...
char32_t getchar32()
{
    ...
    while (true)
    {
        ...
        if (record.EventType == KEY_EVENT && record.Event.KeyEvent.bKeyDown)
        {
            ...
            { // Check if wc is a high surrogate
                high_surrogate = wc;
                continue;
            }
            else if ((wc >= 0xDC00) && (wc <= 0xDFFF))
            { // Check if wc is a low surrogate
                if (high_surrogate != 0)
                { // Check if we have a high surrogate
    ...
    { // Check if wc is a high surrogate
        ...
        { // Check if the next wchar is a low surrogate

...
#define KEY_USE_GELU            "siglip.use_gelu"
#define KEY_N_EMBD              "siglip.%s.embedding_length"
#define KEY_N_FF                "siglip.%s.feed_forward_length"
#define KEY_N_BLOCK             "siglip.%s.block_count"
#define KEY_N_HEAD              "siglip.%s.attention.head_count"
#define KEY_LAYER_NORM_EPS      "siglip.%s.attention.layer_norm_epsilon"
#define KEY_PROJ_DIM            "siglip.%s.projection_dim"
#define KEY_TOKENS              "tokenizer.ggml.tokens"
#define KEY_N_POSITIONS         "clip.text.context_length"
#define KEY_IMAGE_SIZE          "siglip.vision.image_size"
#define KEY_PATCH_SIZE          "siglip.vision.patch_size"
#define KEY_IMAGE_MEAN          "siglip.vision.image_mean"
#define KEY_IMAGE_STD           "siglip.vision.image_std"
#define KEY_PROJ_TYPE           "siglip.projector_type"

#define KEY_MM_PATCH_MERGE_TYPE   "siglip.vision.mm_patch_merge_type"
#define KEY_IMAGE_GRID_PINPOINTS  "siglip.vision.image_grid_pinpoints"
#define KEY_IMAGE_CROP_RESOLUTION "siglip.vision.image_crop_resolution"
...
struct clip_vision_model {
    ...
    // omni-vlm projection
    struct ggml_tensor * mm_head_attn_in_w; // shape is [hidden_dim, 3*hidden_dim]
    struct ggml_tensor * mm_head_attn_in_b;
    struct ggml_tensor * mm_head_attn_out_w;
    struct ggml_tensor * mm_head_attn_out_b;
    struct ggml_tensor * mm_head_ln_w;
    struct ggml_tensor * mm_head_ln_b;
    struct ggml_tensor * mm_head_ffd_w;
    struct ggml_tensor * mm_head_ffd_b;
    struct ggml_tensor * mm_head_ffu_w;
    struct ggml_tensor * mm_head_ffu_b;
    struct ggml_tensor * mm_head_prob;
...
struct clip_ctx * clip_model_load(const char * fname, const char * omni_vlm_version, const int verbosity = 1) {
    ...
    // model size and capabilities
    {
        ...
        // }
        new_clip->has_llava_projector = true;

        // idx = gguf_find_key(ctx, KEY_HAS_MINICPMV_PROJ);
        // if (idx != -1) {
        //     new_clip->has_minicpmv_projector = gguf_get_val_bool(ctx, idx);
        // }
        new_clip->has_minicpmv_projector = false;
        ...
        new_clip->use_gelu = true; // TODO: to be confirmed
    ...
    if (new_clip->has_vision_encoder) {
        ...
        */
        ...
        if (new_clip->proj_type == PROJECTOR_TYPE_MLP || new_clip->proj_type == PROJECTOR_TYPE_MLP_NORM) {
            ...
            // TODO: omni-vlm patch code here:
            vision_model.mm_head_attn_in_w = get_tensor(new_clip->ctx_data, "v.head.attention.in_weight");
            vision_model.mm_head_attn_in_b = get_tensor(new_clip->ctx_data, "v.head.attention.in_bias");
            vision_model.mm_head_attn_out_w = get_tensor(new_clip->ctx_data, "v.head.attention.out.weight");
            vision_model.mm_head_attn_out_b = get_tensor(new_clip->ctx_data, "v.head.attention.out.bias");
            vision_model.mm_head_ln_w = get_tensor(new_clip->ctx_data, "v.head.ln.weight");
            vision_model.mm_head_ln_b = get_tensor(new_clip->ctx_data, "v.head.ln.bias");
            vision_model.mm_head_ffd_w = get_tensor(new_clip->ctx_data, "v.head.ffn_down.weight");
            vision_model.mm_head_ffd_b = get_tensor(new_clip->ctx_data, "v.head.ffn_down.bias");
            vision_model.mm_head_ffu_w = get_tensor(new_clip->ctx_data, "v.head.ffn_up.weight");
            vision_model.mm_head_ffu_b = get_tensor(new_clip->ctx_data, "v.head.ffn_up.bias");
            vision_model.mm_head_prob = get_tensor(new_clip->ctx_data, "v.head.probe");
...
bool clip_image_preprocess(struct clip_ctx * ctx, const clip_image_u8 * img, clip_image_f32_batch * res_imgs) {
    // develop for omni-vlm, WIP
    // TODO: a better structure
    res_imgs->data = new clip_image_f32[1];
    res_imgs->size = 1;

    clip_image_u8 sampled_img;
    bool ret = bicubic_resize(*img, sampled_img, ctx->vision_model.hparams.image_size, ctx->vision_model.hparams.image_size);

    clip_image_f32 img_f32;
    normalize_image_u8_to_f32(&sampled_img, &img_f32, ctx->image_mean, ctx->image_std);

    res_imgs->data[0] = img_f32;

    return true;

    // result is not exactly same compared to PIL.resample(bicubic)
    // need to carefully assess the impact

    // cout << "\t DEBUG DEBUG SAMPLED IMAGE" << endl;
    // cout << sampled_img.nx << '\t' << sampled_img.ny << endl;
    // cout << sampled_img.buf.size() << endl;
    // cout << "\t\tout" << endl;
    // for(size_t i=200000; i<200050;i++) {
    //     cout << (int)(sampled_img.buf[i]) << ' ';
    // }
    // cout << endl;
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs, float * vec) {
...

    {
        ...
        // std::ifstream ifs("/Users/liwei/repo/llama-cpp-experiments/huggingface/pixel_values");
        // ifs.read(reinterpret_cast<char*>(data), 384*384*3*sizeof(float));
        // cout << "\t\tGGGET" << endl;
        // ifs.close();

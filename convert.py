...
DT_Q4_2 = QuantizedDataType(groupsize=128, have_addends=True, have_g_idx=False) #change groupsize it
...
class GGMLFileType(enum.Enum):
    ...
    MostlyQ4_2 = 5 # except 1d tensors
    PerLayerIsQ4_2 = 6 # but tok_embeddings.weight and output.weight are F16
    def type_for_tensor(self, name: str, tensor: 'LazyTensor') -> DataType:
        if len(tensor.shape) == 1:
            ...
            if name in ('output.weight', 'tok_embeddings.weight'):
                ...
                return DT_Q4_1
        elif self == GGMLFileType.MostlyQ4_2:
            return DT_Q4_2
        elif self == GGMLFileType.PerLayerIsQ4_2:
            if name in ('output.weight', 'tok_embeddings.weight'):
                return DT_F16
            else:
                return DT_Q4_2
...
class GGMLQuantizedTensor(Tensor):
    ...
    def __init__(self, ndarray: NDArray, shape: list[int], data_type: DataType) -> None:
        rows, columns = shape
        assert data_type in (DT_Q4_2, DT_Q4_1, DT_Q4_0) # for now
        ...
        self.ndarray = ndarray.view(dtype=np.uint32).reshape((rows, columns // data_type.groupsize, -1))
        self.shape = shape[:]
...
class GPTQForLLaMaQuantizedTensor(Tensor):
    ...
    def to_ggml(self) -> GGMLQuantizedTensor:
        ...
        grouped = self.qweight.reshape([self.qweight.shape[0], self.addends.shape[1], -1])
        # And concatenate:
        grouped = np.concatenate([scales_view, addends_view, grouped], axis=2, casting='no')

        return GGMLQuantizedTensor(grouped, self.shape, DT_Q4_2)
...
def lazy_load_ggml_file(fp: IO[bytes], path: Path) -> ModelPlus:
    ...
    def read_tensor() -> None: # this is a function so that variables captured in `load` don't change
        ...
        if data_type == DT_Q4_1 or data_type == DT_Q4_2:
...
def pick_output_type(model: LazyModel, output_type_str: Optional[str]) -> GGMLFileType:
    ...
    if output_type_str == "q4_2" or (output_type_str is None and isinstance(wq_type, QuantizedDataType) and
                                     wq_type.have_addends):
        if isinstance(model["output.weight"].data_type, QuantizedDataType):
            return GGMLFileType.MostlyQ4_2
        else:
            return GGMLFileType.PerLayerIsQ4_2
...
def main(args_in: Optional[list[str]] = None) -> None:
    ...
    parser.add_argument("--outtype", choices=["f32", "f16", "q4_1", "q4_2"], help="output format (default: based on input)")

...
class Model(ABC):
    ...
    def write_tensors(self):
        ...
        for name, data_torch in self.get_tensors():
            ...
            if (
                self.ftype == 1
                and data_dtype == np.float16
                and (n_dims == 1 or new_name.endswith("_norm.weight"))
            ):
                data = data.astype(np.float32)
    ...
    def write_tensors(self):
        ...
        for name, data_torch in model_kv.items():
            ...
            if name.endswith(("q_proj.weight")):
                data_torch = self._reverse_hf_permute(
                    data_torch, head_count, head_count
                )
            if name.endswith(("k_proj.weight")):
                data_torch = self._reverse_hf_permute(
                    data_torch, head_count, head_count_kv
                )
                ...
                if len(experts) >= n_experts:
                    ...
                    for bid in range(block_count):
                        for wid in range(1, 4):
                            ...
                            merged_name = (
                                f"layers.{bid}.feed_forward.experts.w{wid}.weight"
                            )

                            new_name = tensor_map.get_name(
                                merged_name, try_suffixes=(".weight", ".bias")
                            )
                            if new_name is None:
                                print(f"Can not map tensor {name!r}")
                                sys.exit()
                            ...
                            print(
                                f"{new_name}, n_dims = {len(data.shape)}, shape = {data.shape} --> {data.dtype}"
                            )
                ...
                if len(experts) >= n_experts:
                    ...
                    for bid in range(block_count):
                        for wid in ["linear", "linear_1", "linear_v"]:
                            ...
                            merged_name = (
                                f"transformer.decoder_layer.{bid}.moe.{wid}.weight"
                            )
...
class MambaModel(Model):
    ...
    def write_tensors(self):
        ...
        for name, data_torch in self.get_tensors():
            ...
            new_weight_name = (
                new_name[: -len(".weight")] if new_name.endswith(".weight") else ""
            )
            if (
                self.ftype == 1
                and data_dtype == np.float32
                and new_weight_name.endswith(

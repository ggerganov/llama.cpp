...
static __device__ __forceinline__ float vec_dot_iq2_xxs_q8_1(
    ...
    const float d = (float)bq2->d * (0.5f + aux32) * (float)reinterpret_cast<const half&>(bq8_1[ib32].ds.x) * 0.25f;
    return d * sumi;
#else
    // iqs is 0...15
    ...
    const uint32_t aux32 = q2[2] | (q2[3] << 16);
    const float d = (float)bq2->d * (0.5f + (aux32 >> 28)) * (float)reinterpret_cast<const half&>(bq8_1[ib32].ds.x) * 0.25f;
...
static __device__ __forceinline__ float vec_dot_iq2_xs_q8_1(
    ...
    const float d = (float)bq2->d * (float)reinterpret_cast<const half&>(bq8_1[ib32].ds.x) * 0.25f;
